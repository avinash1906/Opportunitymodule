...
import com.Vtiger.ObjectRepLib.Opp32;
import com.Vtiger.genericlib.BaseClass;
import com.Vtiger.genericlib.FileLib;
import com.Vtiger.genericlib.WebdriverCommonUtile;

public class OpportunityModuleTC32 extends BaseClass {

	// search opportunity by opportunity no
	@Test
	public void serachOpportunityNo() throws Throwable {
		...
		Opp32 op = PageFactory.initElements(BaseClass.driver, Opp32.class);
		op.getUser_name().sendKeys("admin");
		op.getPassword().sendKeys("manager");
		op.getSubmit().click();
System.out.println("hhhhh ansudf now i");
		...
		op.getSearchBox().sendKeys("POT1");

		// enter opportunity no
		WebElement wb = op.getOpportunityno();
		Select sel = new Select(wb);
		// select opportunity no from dropdown "in"
		sel.selectByVisibleText("Opportunity No");
