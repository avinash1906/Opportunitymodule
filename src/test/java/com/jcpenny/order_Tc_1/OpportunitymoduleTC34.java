...
public class OpportunitymoduleTC34 extends BaseClass {
	// search opportunity by opportunity related to

	// login to Vtiger CRM
	@Test
	public void searchRelatedTo() throws Throwable {
		 ...
		 * 
		 * } catch(Exception e) { System.out.print("Caught NullPointerException"); }
		 */
		// login to CRM with Valid data

		Opp34 op = PageFactory.initElements(BaseClass.driver, Opp34.class);
		...
		op.getSearchBox().sendKeys("Qspiders12");

		// enter opportunity related to
		WebElement wb = op.getSearchField();
		;
		Select sel = new Select(wb);
		// select opportunity no from dropdown "in"
		sel.selectByVisibleText("Related To");
